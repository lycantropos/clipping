...
from clipping.core.bounding import is_subset_of
...
def test_basic(boxes_pair: BoxesPair) -> None:
    ...
    result = is_subset_of(left_box, right_box)
...
def test_reflexivity(box: Box) -> None:
    assert is_subset_of(box, box)


@given(strategies.boxes_pairs)
def test_antisymmetry(boxes_pair: BoxesPair) -> None:
    left_box, right_box = boxes_pair

    assert implication(is_subset_of(left_box, right_box)
                       and is_subset_of(right_box, left_box),
                       left_box == right_box)
...
def test_transitivity(boxes_triplet: BoxesTriplet) -> None:
    ...
    assert implication(is_subset_of(left_box, mid_box)
                       and is_subset_of(mid_box, right_box),
                       is_subset_of(left_box, right_box))
